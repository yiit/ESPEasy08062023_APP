...
#ifdef RS232
bool RS232_AKTIF = true;
#else HAL_ESP32_HAL_H_
#include <BLEDevice.h>
#endif
...
#include "Thermal_Label_Printer.h"
...
const char *szBLENames[] = {(char *)"PT-210", (char *)"MTP-3",(char *)"MTP-3F",(char *)"GT01",(char *)"GT02",(char *)"GB01",(char *)"GB02", (char *)"YHK-A133", (char *)"PeriPage+",(char *)"PeriPage_",NULL};
...
const int iPrinterWidth[] = {384, 576, 384, 576, 384, 768, 768};
...
uint8_t *s, *d, bits, ucMask, ucClr, uc;
GFXglyph glyph, *pGlyph;
uint8_t ucTemp[1024]; // max width of 1 scan line (576 pixels)
...
//
int tpScan(const char *szName, int iSeconds, bool rs232)
{
...
#ifdef RS232  
   RS232_AKTIF = rs232;
   ucPrinterType = RONGTA_RP410;
   bFound = 1;
   bConnected = true;
   Serial.println("ok");
   Serial.println(ucPrinterType);
#else HAL_ESP32_HAL_H_
...
int i;
  ...
  if (ucPrinterType == PRINTER_MTP2 || ucPrinterType == PRINTER_MTP3 || ucPrinterType == PRINTER_PERIPAGE || ucPrinterType == PRINTER_PERIPAGEPLUS || ucPrinterType == RONGTA_RP410) {
     i = 0;
...
uint8_t ucTemp[16];
  ...
  } else if (ucPrinterType == PRINTER_MTP2 || ucPrinterType == PRINTER_MTP3 || ucPrinterType == RONGTA_RP410) {
...
static void tpSendScanline(uint8_t *s, int iLen)
{
  if (ucPrinterType == PRINTER_CAT) {
      ...
      ucTemp[5] = 0;
      for (i_lp=0; i_lp<iLen; i_lp++) { // reverse the bits
        ucTemp[6+i_lp] = ucMirror[s[i_lp]];
      } // for each byte to mirror
      ucTemp[6 + iLen] = 0;
      ucTemp[6 + iLen + 1] = 0xff;
      ucTemp[6 + iLen] = CheckSum(&ucTemp[6], iLen);
      tpWriteData(ucTemp, 8 + iLen);
  } else if (ucPrinterType == PRINTER_MTP2 || ucPrinterType == PRINTER_MTP3 || ucPrinterType == PRINTER_PERIPAGE || ucPrinterType == PRINTER_PERIPAGEPLUS || ucPrinterType == RONGTA_RP410) {
      tpWriteData(s, iLen);
  }
