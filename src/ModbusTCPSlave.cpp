...
#include "src/Globals/Logging.h"  //  For serialWriteBuffer
...
#endif
  while (Mbclient.available()) {
    flagClientConnected = 1;
    hataTimer_l = millis();
    int i = 0;
    while (Mbclient.available() > 0) {
      ByteArray[i] = Mbclient.read();
      i++;
    }
#ifdef MB_ESP8266
    Mbclient.flush();
    ...
    Start_Modbus = word(ByteArray[MB_TCP_REGISTER_START], ByteArray[MB_TCP_REGISTER_START + 1]);
  ...
  switch (byteFN) {
    ...
    case MB_FC_READ_REGISTERS:  // 03 Read Holding Registers
      ...
      for (int i = 0; i < WordDataLength; i++) {
        ByteArray[9 + i * 2] = highByte(MBHoldingRegister[Start_Modbus + i]);
        ByteArray[10 + i * 2] = lowByte(MBHoldingRegister[Start_Modbus + i]);
      }
    ...
    case MB_FC_READ_INPUT_REGISTERS:  // 04 Read Input Registers
      ...
      ByteArray[8] = ByteDataLength;      //Number of bytes after this one (or number of bytes of data).
      for (int i = 0; i < WordDataLength; i++) {
        ByteArray[9 + i * 2] = highByte(MBInputRegister[Start_Modbus + i]);
        ByteArray[10 + i * 2] = lowByte(MBInputRegister[Start_Modbus + i]);
      }
      MessageLength = ByteDataLength + 9;
      Mbclient.write((const uint8_t *)ByteArray, MessageLength);
      /*#ifdef MBDebug
    ...
    case MB_FC_WRITE_REGISTER:  // 06 Write Holding Register
      MBHoldingRegister[Start_Modbus] = word(ByteArray[MB_TCP_REGISTER_NUMBER], ByteArray[MB_TCP_REGISTER_NUMBER + 1]);
      ...
      Serial1.print(Start_Modbus);
      Serial1.print("=");
      Serial1.println(MBHoldingRegister[Start_Modbus]);
#endif*/
    ...
    case MB_FC_WRITE_MULTIPLE_REGISTERS:  //16 Write Holding Registers
      ...
      ByteArray[5] = ByteDataLength + 3;  //Number of bytes after this one.
      for (int i = 0; i < WordDataLength; i++) {
        MBHoldingRegister[Start_Modbus + i] = word(ByteArray[13 + i * 2], ByteArray[14 + i * 2]);
      }
      MessageLength = 12;
      Mbclient.write((const uint8_t *)ByteArray, MessageLength);
      /*#ifdef MBDebug
