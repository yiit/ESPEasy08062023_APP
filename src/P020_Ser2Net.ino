    ...
    case PLUGIN_INIT:
      {
        ...
        // serial0 on esp32 is Ser2net: port=2 rxPin=3 txPin=1; serial1 on esp32 is Ser2net: port=4 rxPin=13 txPin=15; Serial1 on esp32 is
        ...
        task->ser2netSerial->println("\n");
        delay(20);
        task->ser2netSerial->println("satir1sil");
        task->ser2netSerial->println("satir2sil");
        task->ser2netSerial->println("satir3sil");
        delay(2000);
        task->ser2netSerial->println("satir1yaz  HRCMESAJ  ");
        task->ser2netSerial->println("satir2yaz   96x48    ");
        task->ser2netSerial->println("satir3yaz  VER: 4.2  ");
        delay(2000);
        task->ser2netSerial->println("satir1sil");
        task->ser2netSerial->println("satir2sil");
        task->ser2netSerial->println("satir3sil");
        delay(200);
        task->ser2netSerial->print("satir1yaz  ");
        task->ser2netSerial->print(String(static_cast<int>(NetworkLocalIP()[0])));
        task->ser2netSerial->print(".");
        task->ser2netSerial->print(String(static_cast<int>(NetworkLocalIP()[1])));
        task->ser2netSerial->println();
        task->ser2netSerial->print("satir2yaz  ");
        task->ser2netSerial->print(String(static_cast<int>(NetworkLocalIP()[2])));
        task->ser2netSerial->print(".");
        task->ser2netSerial->print(String(static_cast<int>(NetworkLocalIP()[3])));
        task->ser2netSerial->println();
        task->ser2netSerial->print("satir3yaz PORT: ");
        task->ser2netSerial->print(String(P020_SERVER_PORT));
        task->ser2netSerial->println();
        delay(2000);
        task->ser2netSerial->println("satir1sil");
        task->ser2netSerial->println("satir2sil");
        task->ser2netSerial->println("satir3sil");
        delay(200);        
        /*task->ser2netSerial->println("sil");
        delay(2000);
        task->ser2netSerial->println("mesHRCMESAJ ");
        delay(2000);
        task->ser2netSerial->println("sil");
        delay(200);
        task->ser2netSerial->println("mesVER: 4.2 ");
        delay(2000);
        task->ser2netSerial->println("sil");
        delay(200);
        task->ser2netSerial->print("mes");
        task->ser2netSerial->print(String(static_cast<int>(NetworkLocalIP()[0])));
        task->ser2netSerial->print(".");
        task->ser2netSerial->print(String(static_cast<int>(NetworkLocalIP()[1])));
        task->ser2netSerial->println();
