...
float                      p078_readVal(uint8_t      query,
                                        uint8_t      node,
                                        unsigned int model);
int                        p078_readintVal(uint8_t query, uint8_t node, unsigned int model);
...
long p078_readLongVal(uint8_t query, uint8_t node, unsigned int model) {
  if (Plugin_078_SDM == nullptr) return 0.0f;
  ...
  while (retry_count > 0 && !success) {
    Plugin_078_SDM->clearErrCode();
    _tempvar = Plugin_078_SDM->readintVal(p078_getRegister(query, model), node);
    --retry_count;
    if (Plugin_078_SDM->getErrCode() == SDM_ERR_NO_ERROR) {
      success = true;
    }
...
unsigned int p078_getRegister(uint8_t query, uint8_t model) {
  if (model == 0) { // SDM120C
    switch (query) {
      case 0: return 0x09DF;//SDM120C_VOLTAGE;
      case 1: return 0x09E0;//SDM120C_CURRENT;
