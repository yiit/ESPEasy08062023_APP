...
#if defined(USES_P130) || defined(USES_P131) || defined(USES_P132) || defined(USES_P133)
...
bool executeInternalCommand(command_case_data & data)
{
  ...
  switch (data.cmd_lc[0]) {
    ...
    #if defined(USES_P130) || defined(USES_P131) || defined(USES_P132) || defined(USES_P133)
      COMMAND_CASE_A("eyztest", Command_Eyz_Test, 1);             // EYZ.h
      COMMAND_CASE_A("eyzart", Command_Eyz_Art, 1);               // EYZ.h
      COMMAND_CASE_A("eyztek", Command_Eyz_Tek, 1);               // EYZ.h
      COMMAND_CASE_A("eyzsensor", Command_Eyz_Sensor, 0);         // EYZ.h
      COMMAND_CASE_A("eyzserino", Command_Eyz_Serino, 1);         // EYZ.h
      COMMAND_CASE_A("eyzpluart", Command_Eyz_Plu_Art, 3);        // EYZ.h
      COMMAND_CASE_A("eyzyaz", Command_Eyz_Yaz, 30);              // EYZ.h
      COMMAND_CASE_A("eyzyazsatir", Command_Eyz_Yaz_Satir, 1);    // EYZ.h
      COMMAND_CASE_A("eyznet?", Command_Eyz_Net, 0);              // EYZ.h
      COMMAND_CASE_A("eyztspl", Command_Eyz_tspl, 1);             // EYZ.h
    #endif  
    #ifdef ESP_NOW_ACTIVE    
      COMMAND_CASE_R("espnowpair", Command_Espnow_Pair,    0);               // Rule.h
    #endif
      break;
    }
    ...
    case 'f':
      {
        COMMAND_CASE_A("fyztest", Command_Fyz_Test, 1);           // FYZ.h
        COMMAND_CASE_A("fyzart", Command_Fyz_Art, 2);             // FYZ.h
        COMMAND_CASE_A("fyztop", Command_Fyz_Top, 1);             // FYZ.h
        COMMAND_CASE_A("fyzkop", Command_Fyz_Kopya, 1);           // FYZ.h
        COMMAND_CASE_A("fyzpluart", Command_Fyz_Plu_Art, 3);      // FYZ.h
        COMMAND_CASE_A("fyzyaz", Command_Fyz_Yaz, 10);            // FYZ.h
        COMMAND_CASE_A("fyzyazsatir", Command_Fyz_Yaz_Satir, 1);  // FYZ.h
        COMMAND_CASE_A("fyznet?", Command_Fyz_Net, 0);            // FYZ.h
        COMMAND_CASE_A("fyzescpos", Command_Fyz_Esc_Pos, 1);      // FYZ.h
        break;
      }
    #endif
    ...
    case 'h': {
      ...
      //#ifdef USES_P110
        //COMMAND_CASE_R("hrcsettings", Command_Hrc_Settings, 0);  // HRC.h
      //#endif
      COMMAND_CASE_A("hedef1", Command_Hedef1_Settings, 0);  // Settings.h
      #ifdef USES_P110
      COMMAND_CASE_A("hrcmesaj", Command_Hrc_Mesaj, 1);       // Hrc.h
      #endif
      break;
    }
...
#ifdef USES_P140
      COMMAND_CASE_R("indsifirkal", Command_Ind_Sifir_Kal, 0);  // IND.h
      COMMAND_CASE_R("indyukkal"  , Command_Ind_Yuk_Kal,   1);  // IND.h
      COMMAND_CASE_R("indsifir"   , Command_Ind_Sifir,     0);  // IND.h
      COMMAND_CASE_R("indelledara", Command_Ind_Elle_Dara, 1);  // IND.h
      COMMAND_CASE_R("inddaraekle", Command_Ind_Dara_Ekle, 0);  // IND.h
      COMMAND_CASE_R("inddarasil" , Command_Ind_Dara_Sil,  0);  // IND.h
      COMMAND_CASE_R("indseridata", Command_Ind_Seri_Data, 0);  // IND.h
#endif
      break;
    }
    case 'j': {
    ...
    case 'n': {
      ...
      COMMAND_CASE_A("net?"   , Command_Srvnet, 0); // Tasks.h
#if FEATURE_NOTIFIER
    ...
    case 't': {
      if (data.cmd_lc[1] == 'a') {
        COMMAND_CASE_R("tare", Command_Settings_Tare, 0);  // settings.h
    ...
    case 'z':
    {
        COMMAND_CASE_R("zero", Command_Settings_Zero, 0);  // settings.h
        break;
    }
    default:
      break;
...
// Yeni yardımcı: Komutun textual cevabını 'reply' içine yazar.
// alsoSendStatus=true: mevcut davranışı korur, SendStatus(...) da yapar.
// InternalCommands.cpp içine ekle
bool ExecuteCommand_internal_withReply(EventValueSource::Enum source,
                                       const char *Line,
                                       String &reply,
                                       bool alsoSendStatus /*= true*/)
{
  String cmd;
  EventStruct TempEvent;

  if (!GetArgv(Line, cmd, 1)) {
    reply = return_command_failed();
    if (alsoSendStatus) SendStatus(&TempEvent, reply);
    return false;
  }
  ...
  String action(Line);
  action = parseTemplate(action);

  command_case_data data(cmd.c_str(), &TempEvent, action.c_str());
  bool handled = executeInternalCommand(data);

  if (data.status.length() > 0) {
    reply = data.status;               // <-- gerçek komut cevabı
    if (alsoSendStatus) SendStatus(&TempEvent, data.status);
  }

  if (handled) {
    return true;                       // internal komut işlendi
  }

  // Internal değilse "unknown" de; plugin/remote YOK
  reply = F("Command unknown: ");
  reply += action;
  if (alsoSendStatus) SendStatus(&TempEvent, reply);
  return false;
}
