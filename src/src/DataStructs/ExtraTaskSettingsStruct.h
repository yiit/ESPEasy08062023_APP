...
struct ExtraTaskSettingsStruct
{
  ...
  bool        TaskPrintBartender;
  uint8_t     TaskDevicePrint[10];
  uint8_t     TaskDeviceIsaretByte;
  uint8_t     TaskDeviceSonByte;
  uint8_t     TaskDeviceValueBas[VARS_PER_TASK];
  uint8_t     TaskDeviceValueBit[VARS_PER_TASK];
  char        TaskDeviceMesage[10][16];
#ifdef CAS_VERSION  
  char        TaskDeviceBaslik[5][42];
#endif  
};
