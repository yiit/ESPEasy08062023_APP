...
void processDisconnect() {
  ...
  if (WiFiEventData.lastConnectedDuration_us > 0 && (WiFiEventData.lastConnectedDuration_us / 1000) < 5000) {
    if (!WiFi_AP_Candidates.getBestCandidate().usable())
      addLog(LOG_LEVEL_INFO, F("WIFI : !getBestCandidate().usable()  => mustRestartWiFi = true")); //degisti

      mustRestartWiFi = true;
  }
  
  if (WiFi.status() == WL_IDLE_STATUS) {
    addLog(LOG_LEVEL_INFO, F("WIFI : WiFi.status() == WL_IDLE_STATUS  => mustRestartWiFi = true")); //degisti
    mustRestartWiFi = true;
  }
...
//    mustRestartWiFi = true;
  ...
  #endif
   //WifiDisconnect(); // degisti Needed or else node may not reconnect reliably.

  if (mustRestartWiFi) {
    WiFiEventData.processedDisconnect = true;
    resetWiFi();
    //WifiScan(false);  //degisti
    //delay(100);  //degisti
    //setWifiMode(WIFI_OFF); //degisti
    //initWiFi();  //degisti
    //delay(100);  //degisti
  }
  //delay(500);
...
void processGotIP() {
  ...
  if ((WiFiEventData.WiFiConnected() || WiFi.isConnected()) && hasIPaddr()) {
    ...
    #ifdef HTTP_EVREKA
    HTTPClient http_evreka; //değişti HTTP evreka
...
# ifndef BUILD_NO_DEBUG
  ...
  if (WiFi_AP_Candidates.addedKnownCandidate() && !NetworkConnected()) {
    if (!WiFiEventData.wifiConnectInProgress) {
      ...
      //setSTA(false);  //degisti
      //NetworkConnectRelaxed();  //degisti
#ifdef USES_ESPEASY_NOW
