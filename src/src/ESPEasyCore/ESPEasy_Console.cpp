...
void EspEasy_Console_t::begin(uint32_t baudrate)
{
  ...
  addLog(LOG_LEVEL_INFO, F("ENFi console using ENFiSerial"));
#else // if FEATURE_DEFINE_SERIAL_CONSOLE_PORT
# ifdef ESP8266
    _serial->begin(baudrate);
    addLog(LOG_LEVEL_INFO, F("ENFi console using HW Serial"));
# endif // ifdef ESP8266
# ifdef ESP32
...
# endif // ifdef ESP32
  ...
  if (_serial_fallback != nullptr) {
    ...
    addLog(LOG_LEVEL_INFO, F("ENFi console fallback enabled"));
  }
#endif // if USES_ESPEASY_CONSOLE_FALLBACK_PORT
}
