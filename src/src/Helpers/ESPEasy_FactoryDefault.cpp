...
void ResetFactory(bool formatFS)
{
  ...
  if (!ResetFactoryDefaultPreference.keepNetwork()) {
    ...
    #if FEATURE_ETHERNET
      str2ip((char *)DEFAULT_IP,     Settings.ETH_IP);
      str2ip((char *)DEFAULT_DNS,    Settings.ETH_DNS);
      str2ip((char *)DEFAULT_GW,     Settings.ETH_Gateway);
      str2ip((char *)DEFAULT_SUBNET, Settings.ETH_Subnet);
    #else
