...
void run_compiletime_checks() {
  ...
  check_size<SecurityStruct,                        606u>();//593u>();
  #ifdef ESP32
  constexpr unsigned int SettingsStructSize = (340 + 84 * TASKS_MAX + 664 + 268);
  #endif
  #ifdef ESP8266
  constexpr unsigned int SettingsStructSize = (316 + 84 * TASKS_MAX + 568);
  #endif
  #if FEATURE_CUSTOM_PROVISIONING
  check_size<ProvisioningStruct,                    256u>();  
  #endif
  check_size<SettingsStruct,                        SettingsStructSize>();
  check_size<ControllerSettingsStruct,              828u>();
  #if FEATURE_NOTIFIER
  check_size<NotificationSettingsStruct,            996u>();
  #endif // if FEATURE_NOTIFIER
  #ifdef ESP8266
  check_size<ExtraTaskSettingsStruct,               524u>();
  #endif
  #ifdef ESP32
  #ifndef CAS_VERSION 
  check_size<ExtraTaskSettingsStruct,               1728u>() ;//1420u>();//536u>();  
  #else
  check_size<ExtraTaskSettingsStruct,               1936u>();//1628u>();//536u>();
  #endif
  #endif
  ...
  #endif
  #ifdef ESP32
  check_size<C013_SensorInfoStruct,                 866u>();//138u>();
  check_size<C013_SensorDataStruct,                 136u>();//24u>();
  #endif
  #endif
  #ifdef USES_C016
