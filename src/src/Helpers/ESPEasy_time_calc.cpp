...
String formatDateString(const struct tm& ts, char delimiter) {
  ...
  if (delimiter == '\0') {
    sprintf_P(DateString, PSTR("%02d%02d%4d"), ts.tm_mday, ts.tm_mon + 1, year);
  } else {
    sprintf_P(DateString, PSTR("%02d%c%02d%c%4d"), ts.tm_mday, delimiter, ts.tm_mon + 1, delimiter, year);
  }
  return DateString;
}
