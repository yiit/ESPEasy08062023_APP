...
bool SerialBT_CL_Aktif = false;
/*********************************************************************************************\
 * Tasks that run 100 times per second
\*********************************************************************************************/

void run100TimesPerSecond(){
  String dummy;
  {
    START_TIMER;
    PluginCall(PLUGIN_ONE_HUNDRED_PER_SECOND, 0, dummy);
    STOP_TIMER(PLUGIN_CALL_100PS);
  }
}
...
void runOncePerSecond()
{
  ...
  if (Settings.bluetooth_mod == 1) {
    ...
    SerialBT.connect();
    if (SerialBT.disconnect()) {
      Serial1.print((char*)address);
      Serial1.println("Baglanti yok");
      SerialBT.connect(address);
    }
    if (!SerialBT.connected(10)) {
      SerialBT.connect(address);
      Serial1.println(F("Baglandi"));
    }
  }
  if (Settings.bluetooth_mod == 2) {
    if ((SerialBT.hasClient()) && (!SerialBT_CL_Aktif)) {
      SerialBT_CL_Aktif = true;
      addLog(LOG_LEVEL_INFO, F("SerialBT Client online"));
      Serial.println(F("SerialBT Client online"));
      pinMode(bluetooth_led, OUTPUT);
      digitalWrite(bluetooth_led, HIGH);
    }
    if ((!SerialBT.hasClient()) && (SerialBT_CL_Aktif)) {
      SerialBT_CL_Aktif = false;
      addLog(LOG_LEVEL_INFO, F("SerialBT Client offline"));
      Serial.println(F("SerialBT Client offline"));
      SerialBT.end();
      SerialBT.begin(String(Settings.Name), false);
      addLog(LOG_LEVEL_INFO, F("SerialBT Aktif"));
      Serial.println(F("SerialBT Aktif"));
      pinMode(bluetooth_led, OUTPUT);
...
//    addLogMove(LOG_LEVEL_INFO,  ESPEASY_SERIAL_CONSOLE_PORT.getLogString());
...
#ifdef HAS_WIFI
  if (Settings.UseMaxTXpowerForSending()) { //(WifiAPMode)
    if ((getUptimeMinutes() >= 1) && (WiFi.status() == 6)) {//degisti
      ExecuteCommand_internal(EventValueSource::Enum::VALUE_SOURCE_SYSTEM, "wifiapmode");  //degisti
      WifiAPMode = true;
    }
    else if ((WifiAPMode == true) && (WiFi.status() == 3)) { //degisti
      ExecuteCommand_internal(EventValueSource::Enum::VALUE_SOURCE_SYSTEM, "wifistamode");
      WifiAPMode = false;
    }
  }
#endif
  ...
  sendSysInfoUDP(1);
  /*#ifdef HTTP_EVREKA
    HTTPClient http_evreka; //değişti HTTP 
    String http_data = "http://"; 
    http_data += formatIP(NetworkGatewayIP());
    http_data += ":5861/megatarti/?ip=";
    http_data += formatIP(NetworkLocalIP());
    addLog(LOG_LEVEL_INFO, http_data);
    http_evreka.begin(http_data);
    int http_evreka_code = http_evreka.GET();
    if (http_evreka_code > 0) {
      Serial.printf("[HTTP] GET... code: %d\n", http_evreka_code);
      // file found at server
      if(http_evreka_code == HTTP_CODE_OK) {
        String payload = http_evreka.getString();
        addLog(LOG_LEVEL_INFO, payload);
      }
    } 
    else {
      Serial.printf("[HTTP] GET... failed, error: %s\n", http_evreka.errorToString(http_evreka_code).c_str());
      //ESP.restart();
    }
    http_evreka.end();
  #endif*/
