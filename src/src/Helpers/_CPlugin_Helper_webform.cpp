...
const __FlashStringHelper * toString(ControllerSettingsStruct::VarType parameterIdx, bool displayName)
{
  switch (parameterIdx) {
    ...
    case ControllerSettingsStruct::CONTROLLER_SEND_DATA:                return  F("Send Data");
...
void addControllerParameterForm(const ControllerSettingsStruct& ControllerSettings, controllerIndex_t controllerindex, ControllerSettingsStruct::VarType varType) {
  ...
  switch (varType) {
    ...
    case ControllerSettingsStruct::CONTROLLER_SEND_DATA:
    {
#ifdef ESP32
        ...
        while (file) {
          if (!file.isDirectory()) {
            ...
            if (fname.startsWith(F("/cnt")) || fname.startsWith(F("cnt"))) {
...
#endif
        byte choice2 = ControllerSettings.CData[0];
        ...
        optionValues2[8] = 8;
        optionValues2[9] = 9;
        addFormSelector(F("Client Data"), F("cplugin_3_client_data"), 10, options2, optionValues2, choice2);
        addButton(options2[ControllerSettings.CData[0]], F("Data Dizayn Menüsüne Git"));
      break;
    }
...
void saveControllerParameterForm(ControllerSettingsStruct        & ControllerSettings,
  ...
  switch (varType) {
    ...
    case ControllerSettingsStruct::CONTROLLER_MIN_SEND_INTERVAL: break;
