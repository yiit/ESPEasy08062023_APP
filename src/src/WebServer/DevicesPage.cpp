...
void handle_devices_CopySubmittedSettings(taskIndex_t taskIndex, pluginID_t taskdevicenumber)
{
  ...
  for (uint8_t varNr = 0; varNr < valueCount; varNr++)
  {
    ...
    update_whenset_FormItemInt(getPluginCustomArgName(F("TDVBA"), varNr), ExtraTaskSettings.TaskDeviceValueBas[varNr]);
    update_whenset_FormItemInt(getPluginCustomArgName(F("TDVBI"), varNr), ExtraTaskSettings.TaskDeviceValueBit[varNr]);
...
void handle_devicess_ShowAllTasksTable(uint8_t page)
{
  ...
  html_table_header(F("Sno"),    50);
  html_table_header(F("Aktif"), 100);
  html_table_header(F("APP"));
  html_table_header(F("APP İsmi"));
  html_table_header(F("Port"));
  html_table_header(F("Ctr (IDX)"), 100);
  html_table_header(F("GPIO"));
  html_table_header(F("Data"));
...
void handle_devices_TaskSettingsPage(taskIndex_t taskIndex, uint8_t page)
{
  ...
  {
    switch (Settings.TaskDeviceNumber[taskIndex]) {
      case 100:
        if (!SecuritySettings.LisansControlSERiALSERVER) {
          addWideButtonPlusDescription(F("/lisansserialserver"), F("Lisans SERiAL SERVER"), F("yigit@endutek.com.tr"));
          task_button(page, taskIndex);
          return;
        }
        break;
      case 101:
        if (!SecuritySettings.LisansControlSERiALCLiENT) {
          addWideButtonPlusDescription(F("/lisansserialclient"), F("Lisans SERiAL CLiENT"), F("yigit@endutek.com.tr"));
          task_button(page, taskIndex);
          return;
        }
        break;
      case 102:
        if (!SecuritySettings.LisansControlSERiALTCPMODBUSSLAVE) {
          addWideButtonPlusDescription(F("/lisansserialtcpmodbusslave"), F("Lisans SERiAL TCP/MODBUS SLAVE"), F("yigit@endutek.com.tr"));
          task_button(page, taskIndex);
          return;
        }
        break;
      case 104:
        if (!SecuritySettings.LisansControlTCPMODBUSHRCMESAJ) {
          addWideButtonPlusDescription(F("/lisanstcpmodbushrcmesaj"), F("Lisans TCP/MODBUS HRCMESAJ"), F("yigit@endutek.com.tr"));
          task_button(page, taskIndex);
          return;
        }
        break;        
      case 110:
        if (!SecuritySettings.LisansControlHRC) {
          addWideButtonPlusDescription(F("/lisanshrc"), F("Lisans HRC"), F("yigit@endutek.com.tr"));
          task_button(page, taskIndex);
          return;
        }
        break;
      case 120:
        if (!SecuritySettings.LisansControlFYZ) {
          addWideButtonPlusDescription(F("/lisansfyz"), F("Lisans FYZ"), F("yigit@endutek.com.tr"));
          task_button(page, taskIndex);
          return;
        }
        break;
      case 121:
        if (!SecuritySettings.LisansControlFYZPLU) {
          addWideButtonPlusDescription(F("/lisansfyzplu"), F("Lisans FYZPLU"), F("yigit@endutek.com.tr"));
          task_button(page, taskIndex);
          return;
        }
        break;
      case 130:
        if (!SecuritySettings.LisansControlEYZ) {
          addWideButtonPlusDescription(F("/lisanseyz"), F("Lisans EYZ"), F("yigit@endutek.com.tr"));
          task_button(page, taskIndex);
          return;
        }
        break;
      case 131:
        if (!SecuritySettings.LisansControlEYZPLU) {
          addWideButtonPlusDescription(F("/lisanseyzplu"), F("Lisans EYZPLU"), F("yigit@endutek.com.tr"));
          task_button(page, taskIndex);
          return;
        }
        break;
      case 140:
        if (!SecuritySettings.LisansControlIND) {
          addWideButtonPlusDescription(F("/lisansind"), F("Lisans IND"), F("yigit@endutek.com.tr"));
          task_button(page, taskIndex);
          return;
        }
        break;
    ...
    addFormTextBox(F("APP İsmi"), F("TDN"), getTaskDeviceName(taskIndex), NAME_FORMULA_LENGTH_MAX); // ="taskdevicename"

    addFormCheckBox(F("APP Aktif"), F("TDE"), Settings.TaskDeviceEnabled[taskIndex], Settings.isTaskEnableReadonly(taskIndex)); // ="taskdeviceenabled"
...
void task_button(int page, int taskIndex) {
  addFormSeparator(4);
...
void devicePage_show_task_values(taskIndex_t taskIndex, deviceIndex_t DeviceIndex)
{
  ...
  {
    int colCount = 2;
    addFormSubHeader(F("Değişken Tanımları"));
    html_end_table();
    html_table_class_normal();

    // table header
    addHtml(F("<TR><TH style='width:40px;' align='center'>#"));
    //html_table_header(F("Name"),500);
    html_table_header(F("Değişken Adı"));

    if (Device[DeviceIndex].FormulaOption)
    {
      html_table_header(F("Formül"), F("EasyFormula"), 0);
      html_table_header(F("Baslangıç"), 10);
      html_table_header(F("Bitiş"), 10);
      ++colCount;
    }
    ...
    for (uint8_t varNr = 0; varNr < valueCount; varNr++)
    {
      ...
      if (Device[DeviceIndex].FormulaOption) {
        html_TD();
        const String id = getPluginCustomArgName(F("TDVBA"), varNr);  // ="taskdevicevaluebaşlangıç"
        addNumericBox(id, ExtraTaskSettings.TaskDeviceValueBas[varNr], 0, 1024);
      }

      if (Device[DeviceIndex].FormulaOption) {
        html_TD();
        const String id = getPluginCustomArgName(F("TDVBI"), varNr);  // ="taskdevicevaluebitiş"
        addNumericBox(id, ExtraTaskSettings.TaskDeviceValueBit[varNr], 0, 1024);
      }
      ...
      if (Device[DeviceIndex].configurableDecimals())
      {
        ...
        byte choice = Cache.getTaskDeviceValueDecimals(taskIndex, varNr);
        String options[5];
        options[0] = F("0");
        options[1] = F("0.0");
        options[2] = F("0.00");
        ...
        optionValues[3] = 3;
        optionValues[4] = 4;
        addSelector(id, 5, options, optionValues, nullptr, choice);
