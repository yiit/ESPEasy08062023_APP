...
#ifdef FEATURE_TEST
String csvData = "Zaman; Tartim Verisi\n"; // CSV verilerini kaydetmek için
...
<head>
  ...
  <title>Yangın Tüpü Boşaltım Testi</title>
  ...
  <style>
    ...
    #recordButton.stop {background-color: #dc3545; /* Kırmızı */
    }
    #recordButton:hover {opacity: 0.9;}
    #downloadLink {
...
<body>
  ...
  <h2>Tartım Verileri</h2>

  <button id="recordButton" class="start" onclick="toggleRecording()">Kayıt İşlemine Başla</button>
  <p>Son Tartım Verisi: <span id="weightValue">Bekleniyor...</span></p> <!-- Dinamik veri gösterimi için -->
  <canvas id="weightChart" width="400" height="200"></canvas>
  <br><br>
  <a id="downloadLink" href="/downloadXLS" download="tartim_verileri.xls" style="display:none;">Verileri İndir (XLS)</a>
  <a id="downloadImageLink" style="display:none;">Grafiği İndir (PNG)</a>
  ...
  <script>
    ...
    function toggleRecording() {
      ...
      if (recording) {
        // Kayıt işlemi bitirildi, butonu değiştirme ve indirme linkini gösterme
        recordButton.innerText = "Kayıt İşlemine Başla";
        recordButton.classList.remove('stop');
        recordButton.classList.add('start');
        document.getElementById("downloadLink").style.display = "block";
        document.getElementById("downloadImageLink").style.display = "block"; // İndir butonunu göster
        fetch('/stopRecording');
        clearInterval(intervalID); // Veri alma döngüsünü durdur
        recording = false;

        // Grafiğin ekran görüntüsünü almak için
        downloadImage();
      } else {
        // Kayıt işlemi başlatıldı, grafiği sıfırla ve butonu değiştirme
        recordButton.innerText = "Kayıt İşlemini Bitir";
        recordButton.classList.remove('start');
        recordButton.classList.add('stop');
        document.getElementById("downloadLink").style.display = "none";
        document.getElementById("downloadImageLink").style.display = "none"; // İndir butonunu gizle
        resetChart(); // Grafiği sıfırla
        fetch('/startRecording');
        startRecording();
        startTime = new Date(); // Kayıt başlangıç zamanını al
        recording = true;
      }
    }

    function startRecording() {
      // Seri porttan veri almak için AJAX isteği yap
      intervalID = setInterval(function(){
        if (recording) {
          fetch('/getWeight')
            .then(response => response.text())
            .then(data => {
              var weightValue = parseFloat(data.trim()); // Gelen veriyi işliyoruz
              if (!isNaN(weightValue)) {
                var currentTime = new Date();
                var elapsedTime = Math.floor((currentTime - startTime) / 1000); // Geçen zamanı saniye olarak hesapla
                var timeLabel = formatTime(elapsedTime); // Zamanı MM:ss formatına çevir

                // Veri eklenip grafik güncelleniyor
                dataPoints.push({ x: timeLabel, y: weightValue });
                updateChart();
                // Son tartım verisini güncelleme
                document.getElementById("weightValue").innerText = weightValue.toFixed(3) + " kg";
              } else {
                // Veri boş veya geçersizse
                document.getElementById("weightValue").innerText = "Veri Alınamıyor!";
              }
            })
            .catch(error => {
              console.log("Error: ", error);
              document.getElementById("weightValue").innerText = "Bağlantı Hatası!";
            });
        }
      }, 1000); // 1 saniyede bir veri al
    }

    function resetChart() {
      dataPoints = []; // Verileri sıfırla
      updateChart();   // Grafiği sıfırla
    }
    ...
    function updateChart() {
      if (!chart) {
        ...
        chart = new Chart(ctx, {
          type: 'line',
          data: {
            labels: dataPoints.map(point => point.x), // Zamanı (MM:ss) ekle
            datasets: [{
              label: 'Tartım Verisi',
              data: dataPoints.map(point => point.y), // Tartım verisini ekle
    ...
    // Geçen süreyi MM:ss formatında göstermek için bir yardımcı fonksiyon
    ...
    // Grafiği PNG formatında indirme
    function downloadImage() {
      var canvas = document.getElementById('weightChart');
      var downloadLink = document.getElementById('downloadImageLink');
      var image = canvas.toDataURL('image/png').replace('image/png', 'image/octet-stream');
      downloadLink.href = image;
      downloadLink.download = 'grafik.png'; // İndirme dosya adı
    }

    // Sayfa yüklendiğinde veri okumayı başlat
...
const char SCALE_page[] PROGMEM = R"(
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Tartım Terminali (Seven-Segment Kırmızı)</title>
  <!-- Font Yükleme -->
  <style>
    @font-face {
      font-family: 'SevenSegment';
      src: url('/Segment.ttf') format('truetype'); /* Font dosyasının yolu */
    }
    ...
    body {
      ...
      padding: 0;
      display: flex;
      justify-content: center; /* Ekranı yatay olarak ortalar */
      align-items: center; /* Ekranı dikey olarak ortalar */
    ...
    .display-container {
      position: relative;
      width: 600px; /* Görüntünün genişliğini ayarlayın */
      height: 400px; /* Görüntünün yüksekliğini ayarlayın */
      background: url('/scale.jpg') no-repeat center center;
      background-size: contain; /* Görseli alanın içinde tut */
    }

    .seven-segment-display {
      position: absolute;
      top: 40%; /* Yazıyı yukarı kaydırır */
      left: 60%;
      transform: translate(-50%, -50%); /* Metni tam ortalamak için */
      display: flex;
    ...
    .segment {
      font-size: 100px; /* Yazıyı büyütüyoruz */
      color: #FF0000; /* Kırmızı seven-segment LED rengi */
      text-shadow: 0px 0px 10px rgba(255, 0, 0, 0.8); /* LED efekti */
      font-family: 'SevenSegment', sans-serif; /* Yüklenen seven-segment fontu */
    }
  </style>

  <script>
    // Ağırlık verisini XML sayfasından al ve güncelle
    function getWeightData() {
      var xhr = new XMLHttpRequest();
      xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
          var xml = xhr.responseXML;
          var netWeight = xml.getElementsByTagName('net')[0].childNodes[0].nodeValue;
          displayWeight(netWeight);
        }
      };
      xhr.open('GET', '/xml', true); // XML verisini çekmek için '/xml' yolunu kullan
      xhr.send();
    }

    // Veriyi seven-segment LED tarzında göster
    function displayWeight(weight) {
      const weightDisplay = document.getElementById('seven-segment-display');
      weightDisplay.innerHTML = '';  // Önceki veriyi temizle
      for (let i = 0; i < weight.length; i++) {
        let digit = weight[i];
        weightDisplay.innerHTML += `<div class="segment">${digit}</div>`;
      }
    }

    // Sayfa her 100 milisaniyede bir ağırlık verisini günceller
    setInterval(getWeightData, 100);
  </script>
</head>
<body>

  <!-- Görselin ve tartım verisinin bulunduğu alan -->
  <div class="display-container">
    <!-- Tartım verisini göstereceğimiz alan -->
    <div id="seven-segment-display" class="seven-segment-display"></div>
  </div>
...
void WebServerInit()
{
  ...
  web_server.on(F("/blescanner"), handle_blescanner);
#endif // BLE Scanner
  ...
  web_server.on(F("/xml"), handle_xml);
  web_server.on(F("/xml.xml"), handle_xml_xml);
  web_server.on(F("/screen"), handle_screen);

  web_server.on(F("/lisans"), handle_lisans);
  web_server.on(F("/lisansserialserver"), handle_lisans_SERiALSERVER);
  web_server.on(F("/lisansserialclient"), handle_lisans_SERiALCLiENT);
  web_server.on(F("/lisansserialtcpmodbusslave"), handle_lisans_SERiALTCPMODBUSSLAVE);
  web_server.on(F("/lisanstcpmodbushrcmesaj"), handle_lisans_TCPMODBUSHRCMESAJ);
  web_server.on(F("/lisanshrc"), handle_lisans_HRC);
  web_server.on(F("/lisansfyz"), handle_lisans_FYZ);
  web_server.on(F("/lisansfyzplu"), handle_lisans_FYZPLU);
  web_server.on(F("/lisanseyz"), handle_lisans_EYZ);
  web_server.on(F("/lisanseyzplu"), handle_lisans_EYZPLU);
  web_server.on(F("/lisansind"), handle_lisans_IND);
  json_server.on(F("/data"), handle_json_net);

#ifdef FEATURE_TEST
  // Sunucu yapılandırmaları
  web_server.on(F("/test"),handle_test);
  /*web_server.on(F("/test"),[]() {
    web_server.send(200, "text/html", TEST_page);
  });*/

  web_server.on("/getWeight", []() {
    if (recording) {
      // Milisaniyeden saniyeye çevir
      unsigned long saniye = (millis()-zaman) / 1000;
      // Dakikayı ve kalan saniyeyi hesapla
      unsigned int dakika = saniye / 60;  // Tam dakika sayısını bulur
      unsigned int kalanSaniye = saniye % 60;  // Geriye kalan saniyeleri bulur
      char time_c[6];
      sprintf(time_c,"%02d:%02d", dakika, kalanSaniye);
      String XML_NET_S_son = XML_NET_S;
      XML_NET_S_son.replace(".",",");
      csvData += String(time_c) + ";" + XML_NET_S + "\n";
      web_server.send(200, "text/plain", XML_NET_S); // Veriyi gönder
    }
  });

  web_server.on("/stopRecording", []() {
    // Kayıt işlemi durduruluyor
    recording = false;
    web_server.send(200, "text/html", "Kayıt durduruldu.");
  });

  web_server.on("/startRecording", []() {
    recording = true; // Kayıt işlemini başlat
    zaman = millis();
    csvData = "Zaman; Tartim Verisi\n"; // Yeni kayıt için CSV verisini sıfırla
    web_server.send(200, "text/html", "Kayıt başladı.");
  });

  web_server.on("/downloadXLS", []() {
    // CSV dosyasını indir
    web_server.send(200, "application/vnd.ms-excel", csvData); // CSV dosyasını indir ama dosya adı .xls olacak
  });
#endif
...
void setWebserverRunning(bool state) {
  ...
  if (state) {
    ...
    json_server.begin(uint16_t(3000));
...
bool isLisansIn() {
  if (!clientIPallowed()) return false;
  if (!SecuritySettings.LisansControl)
    web_server.sendContent(F("HTTP/1.1 302 \r\nLocation: /lisans \r\n"));
  else
    WebLisansInTimer = 0;
  return SecuritySettings.LisansControl;
}
