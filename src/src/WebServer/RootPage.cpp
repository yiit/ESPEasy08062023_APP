...
void handle_root() {
  ...
  } else if (strcasecmp_P(sCommand.c_str(), PSTR("reset")) == 0)
  {
    ...
    addFormSubHeader(F("Cihaz Yapılandırma"));
    html_TR_TD();
    html_TD();
    //html_BR();
    html_BR();
    ...
    if (!Settings.UseNTP()) {
      ...
      if (web_server.arg(F("datetime")).length() > 0)
        message1 = web_server.arg(F("datetime"));
      else
        message1 = node_time.getDateTimeString('-', ':', '#');
      addFormTextBox(F("Tarih-Saat"), F("datetime"), message1, 19);
      button += message1;
      ...
      addFormNote(F("<font color='red'>Örnek Yazım : 01-01-2023#01:02:00 (gün-ay-yıl#saat:dakika:saniye)</font>"));
    }
    html_end_table();
    html_BR();
    html_BR();
