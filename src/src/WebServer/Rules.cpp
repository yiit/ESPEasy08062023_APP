...
void handle_rules() {
  ...
  if (!isLisansIn() || !Settings.UseRules) { return; }
...
void handle_rules_new() {
  ...
  html_TR();
  html_table_header(F("Dizayn Adı"));
  html_table_header(F("Dosya Yolu"));
  html_table_header(F("Boyut"));
  addHtml(F("<TH>Actions"));
  addSaveButton(F("/rules/backup"), F("Yedek"));
  addButton(F("/upload"), F("Yükle"));
  addHtml(F("</TH></TR>"));

  //class Web_StreamingBuffer &buffer = TXBuffer;
...
#  endif // ifdef WEBSERVER_RULES_DEBUG
      ...
      if (fi.isDirectory)
      {
        ...
        addSaveButton(String(F("/rules/backup?directory=")) + URLEncode(fi.Name), F("Backup"));
      }
      else {
        String encodedPath  = "%2Frules%2F";
        encodedPath +=  URLEncode(String(fi.Name + F(".prn")));
        ...
        // Actions
        html_TD();
        addSaveButton(String(F("/rules/backup?fileName=")) + encodedPath, F("Backup"));

        addDeleteButton(String(F("/rules/delete?fileName=")) + encodedPath, F("Delete"));

        String button;
        if ((Settings.WebAPP == 140) || (Settings.WebAPP == 120) || (Settings.WebAPP == 121) || (Settings.WebAPP == 122))
          button = "fyztest%23";
        else if ((Settings.WebAPP == 140) || (Settings.WebAPP == 130) || (Settings.WebAPP == 131) || (Settings.WebAPP == 132))
          button = "eyztest%23";
        button += encodedPath;
        button.replace("/", "%2f");
        button.replace(".prn", "");
        button += ".prn";
        addPrintButton(String(F("/rules?cmd=")) + button, F("Yazdir"));
      }
      addHtml(F("</TD></TR>"));
#  ifdef WEBSERVER_RULES_DEBUG
...
#  endif // ifdef WEBSERVER_RULES_DEBUG
  ...
  if (startIdx > 0)
  {
    ...
    addButton(String(F("/rules?start=")) + String(showIdx), F("Previous"));
  }

  if (hasMore && (count >= endIdx))
  {
    addButton(String(F("/rules?start=")) + String(endIdx + 1), F("Next"));
  }
...
bool handle_rules_edit(String originalUri, bool isAddNew) {
  ...
  if (isAddNew || (originalUri.startsWith(F("/rules/")) && originalUri.endsWith(F("")))) {
    if (!isLoggedIn() || !Settings.UseRules) { return false; }
    if (!isLisansIn() || !Settings.UseRules) { return false; }
    ...
    if (Settings.WebAPP == 120)
      addFormNote(F("<font color='red'>Dizayn Adı fyz# ile Başlamalı .prn ile Bitmelidir.</font>"));
    else if (Settings.WebAPP == 130)
      addFormNote(F("<font color='red'>Dizayn Adı eyz# ile Başlamalı .prn ile Bitmelidir.</font>"));
    ...
    addButton(F("/devices?index=1&page=1"), F("Terazi/Yazıcı Ayarlarına Git"));
...
bool Rule_Download(const String& path)
{
  ...
  String filename = path.substring(6,path.length()); //+ String(F(".txt"));
  filename.replace(RULE_FILE_SEPARAROR, '#');

  filename.replace("#fyz",    "#fyz#");
  filename.replace("#fyzplu", "#fyzplu#");
  filename.replace("#eyz",    "#eyz#");
  filename.replace("#eyzplu", "#eyzplu#");
  filename.replace("#plu",    "#plu#");
  filename.replace("#logo",   "#logo#");
  filename.replace("#ind",    "#ind#");
  filename.replace("#hrc",    "#hrc#");
  filename.replace("#srv",    "#srv#");
  filename.replace("#cnt",    "#cnt#");
  filename.replace("#cli",    "#cli#");
