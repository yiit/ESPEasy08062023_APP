...
#ifndef MENU_INDEX_SCREEN_VISIBLE
  # define MENU_INDEX_SCREEN_VISIBLE true
#endif // ifndef MENU_INDEX_SCREEN_VISIBLE

#ifndef MENU_INDEX_TEST_VISIBLE
  # define MENU_INDEX_TEST_VISIBLE true
#endif // ifndef MENU_INDEX_SCREEN_VISIBLE
...
const __FlashStringHelper* getGpMenuIcon(uint8_t index) {
  switch (index) {
    ...
    case MENU_INDEX_SCREEN: return F("&#128223;");
#ifdef FEATURE_TEST
    case MENU_INDEX_TEST: return F("&#129519;");  }
#endif
  }  
  return F("");
}
...
const __FlashStringHelper* getGpMenuLabel(uint8_t index) {
  switch (index) {
    case MENU_INDEX_MAIN: return F("Ana");
    case MENU_INDEX_CONFIG: return F("Ayarlar");
    case MENU_INDEX_CONTROLLERS: return F("Controllers");
    case MENU_INDEX_HARDWARE: return F("Donamım");
    case MENU_INDEX_DEVICES: return F("App");
    case MENU_INDEX_RULES: return F("Dizayn");
    case MENU_INDEX_NOTIFICATIONS: return F("Notifications");
    case MENU_INDEX_TOOLS: return F("Araçlar");
    case MENU_INDEX_SCREEN: return F("Ekran");
#ifdef FEATURE_TEST
    case MENU_INDEX_TEST: return F("TEST");
#endif
  }
  return F("");
}
...
const __FlashStringHelper* getGpMenuURL(uint8_t index) {
  switch (index) {
    ...
    case MENU_INDEX_SCREEN: return F("/screen");
#ifdef FEATURE_TEST
    case MENU_INDEX_TEST: return F("/test");
#endif
  }
...
bool GpMenuVisible(uint8_t index) {
  switch (index) {
    ...
    case MENU_INDEX_SCREEN: return MENU_INDEX_SCREEN_VISIBLE;
#ifdef FEATURE_TEST
    case MENU_INDEX_TEST: return MENU_INDEX_TEST_VISIBLE;
#endif
  }
  return false;
}
...
void WebTemplateParser::getWebPageTemplateVar(const String& varName)
{
  ...
  else if (equals(varName, F("menu")))
  {
    ...
    for (uint8_t i = 0; i < 10; i++)
    {
      ...
      if (((i == MENU_INDEX_CONFIG) || (i == MENU_INDEX_DEVICES) || (i == MENU_INDEX_TOOLS)) && !Settings.UseYonetici) {
        continue;
      }
  ...
  {
    if (fileExists(F("logo.png")))
    {
      addHtml(F("<img src=\"logo.png\" width=auto height=auto align=right>"));
    }
  }
